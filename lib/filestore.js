...
async function filestore (fastify, opts = {}) {
  ...
  async function storeFile (fileUri, key = '', opt = {}) {
    const fileArr = fileUri.split(',');
    const fileMime = fileArr.shift().replace('data:', '').split(';')[0].toLowerCase();
    const fileHash = CryptRandStr({ length: opt.length || opts.storeFileLength || 64, type: 'hex' });
    const fileName = `${prefix}/${fileHash}${this.user.id}${Date.now()}${key}.${mime.extension(fileMime)}`;
    const fileBuff = Buffer.from(fileArr.join(), 'base64');
    const plugins = [];
