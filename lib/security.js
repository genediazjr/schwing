...
async function security (fastify, opts) {
  ...
  const { NODE_ENV, CSRFCOOKIE_NAME, JWTCOOKIE_NAME, UIDCOOKIE_NAME, DISCORD_ERRORHOOK, DOMAIN } = config;
  const secure = NODE_ENV.startsWith('prod');
  const head = `X-${CSRFCOOKIE_NAME}`;
  ...
  const getToken = req => req.headers[head] || req.headers[head.toLowerCase()];
  ...
  function repFailed (request, err, obj, opts = {}) {
    ...
    (async () => await postToDiscord(DISCORD_ERRORHOOK, {
      embeds: [{
        title: DOMAIN || 'localhost',
  ...
  async function logout (request, reply) {
    ...
    reply
      .header(head, await reply.generateCsrf({ userInfo }))
  ...
  async function session (request, reply, userObj) {
    ...
    reply
      .header(head, await reply.generateCsrf({ userInfo: hooked.id }))
  ...
  async function onRequest (request, reply) {
    ...
    if (request.method === 'GET' && getToken(request) === '') {
      reply.header(head, await reply.generateCsrf({ userInfo }));
    }
